  ...
  const transformData = (data: any): FinancialInformation => {
    return {
      ...
      annualIncome: data?.annualIncome
        ? parseFloat(String(data?.annualIncome)?.replace(/,/g, ""))
        : 0,
      totalAmountSaved: data?.totalAmountSaved
        ? parseFloat(String(data?.totalAmountSaved)?.replace(/,/g, ""))
        : 0,
      totalLoanAmount: data?.totalLoanAmount
        ? parseFloat(String(data?.totalLoanAmount)?.replace(/,/g, ""))
        : 0,
      totalLoanRepayments: data?.totalLoanRepayments
        ? parseFloat(String(data?.totalLoanRepayments)?.replace(/,/g, ""))
        : 0,
      helpDebtTotalAmount: data?.helpDebtTotalAmount
        ? parseFloat(String(data?.helpDebtTotalAmount)?.replace(/,/g, ""))
        : 0,
      totalExistingHomeLoanAmount: data?.totalExistingHomeLoanAmount
        ? parseFloat(
            String(data?.totalExistingHomeLoanAmount)?.replace(/,/g, "")
          )
        : 0,
      totalExistingHomeLoanRepaymentAmt: data?.totalExistingHomeLoanRepaymentAmt
        ? parseFloat(
            String(data?.totalExistingHomeLoanRepaymentAmt)?.replace(/,/g, "")
          )
        : 0,
      totalPropertyValue: data.totalPropertyValue
        ? parseFloat(String(data.totalPropertyValue)?.replace(/,/g, ""))
        : 0,
      totalCreditCardLimits: data?.totalCreditCardLimits
        ? parseFloat(String(data?.totalCreditCardLimits)?.replace(/,/g, ""))
        : 0,
      livingExpenses: data?.livingExpenses
        ? parseFloat(String(data?.livingExpenses)?.replace(/,/g, ""))
        : 0,
  ...
  return (
    <>
      ...
      >
        ...
        />
        {errors.annualIncome && (
          <FormHelperText>{String(errors.annualIncome.message)}</FormHelperText>
        )}
      </FormControl>
      ...
      <Typography sx={{ marginTop: "20px", fontSize: "14px", fontWeight: 700 }}>
        Monthly living expenses (excludes rent, personal loans, car loans, HECS
        debt)*
      </Typography>
      ...
      <FormControl error={Boolean(errors.hasDefaulted)}>
        ...
        />
        {errors.hasDefaulted && (
          <FormHelperText>{String(errors.hasDefaulted.message)}</FormHelperText>
        )}
      </FormControl>
      {hasDefaulted && (
        <TextField
